...
public class OrderProcessingResource {
	...
	public List<OrderResponseDto> getAllOrders() {
		return session.createQuery( "from Order o join fetch o.orderLines", Order.class )
				.getResultList()
				.stream()
	...
	public OrderResponseDto getOrder(@PathParam("id") Long id) {
		try {
			var order = session.createQuery( "from Order o join fetch o.orderLines where o.id = :id", Order.class )
					.setParameter( "id", id )
					.getSingleResult();
